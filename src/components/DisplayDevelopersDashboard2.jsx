...
export default function DisplayDevelopersDashboard2() {
  ...
  useEffect(() => {   
    ...
    .then((data) => setActiveProjects(data.filter((activeProject) => activeProject.role == 4)));
  }, []);
...
// - calling setAvailableProjects to update State 
  ...
  function regInterestInProject(project_id) {    
    ...
    setAvailableProjects((prevArray) => prevArray.filter((obj) => obj.project_id !== project_id));
    ...
    <>
      <div className="section flex flex-col justify-top items-center border border-black rounded-lg p-2 m-2 bg-yellow-100 h-40 mt-7">
         ...
         {availableProjects.map((availableProject) => {
          return <DevelopersAvailableProject key={availableProject.project_id}  project={availableProject} regInterestInProjectFunc={regInterestInProject} />
        })}
        </ol>
